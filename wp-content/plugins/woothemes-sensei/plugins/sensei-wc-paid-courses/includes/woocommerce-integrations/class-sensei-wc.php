...
class Sensei_WC {
	...
	public static function add_course_archive_wc_filter_links( $filter_links ) {
		...
		$filter_links[] = [
			...
			'title' => __( 'Paid', 'sensei-wc-paid-courses' ),
		];
		...
		$filter_links[] = [
			...
			'title' => __( 'Free', 'sensei-wc-paid-courses' ),
		];
		...
		if ( self::is_course_in_cart( $post->ID ) ) { ?>
			...
			<div class="sensei-message info">
				...
				$cart_link    = '<a class="cart-complete" href="' . esc_url( $checkout_url )
							. '" title="' . esc_attr__( 'complete purchase', 'sensei-wc-paid-courses' ) . '">'
							. esc_html__( 'complete the purchase', 'sensei-wc-paid-courses' ) . '</a>';

				// translators: Placeholder is a link to the cart.
				echo wp_kses_post( sprintf( __( 'You have already added this Course to your cart. Please %1$s to access the course.', 'sensei-wc-paid-courses' ), $cart_link ) );
	...
	public static function purchase_course_default_button_text( $product, $default_text = null ) {
		...
		if ( $product_is_free ) {
			$button_text = __( 'Register for this Course', 'sensei-wc-paid-courses' );
		} else {
			$button_text = __( 'Purchase this Course', 'sensei-wc-paid-courses' );
		}
	...
	public static function the_course_no_permissions_message( $course_id ) {
		...
		$login_link         = '<a href="' . esc_url( get_permalink( $my_courses_page_id ) ) . '">' .
			esc_html__( 'log in', 'sensei-wc-paid-courses' ) . '</a>';
		...
		if ( $any_course_product_in_cart ) {
			...
			$cart_link = '<a href="' . esc_url( wc_get_checkout_url() ) . '" title="' .
				esc_attr__( 'Checkout', 'sensei-wc-paid-courses' ) . '">' . esc_html__( 'checkout', 'sensei-wc-paid-courses' ) .
				'</a>';

			// translators: Placeholder is a link to the cart.
			$message = sprintf( __( 'This course is already in your cart, please proceed to %1$s, to gain access.', 'sensei-wc-paid-courses' ), $cart_link );
			?>
	...
	public static function email_course_details( $order ) {
		...
		if ( 0 < count( $order_items ) ) {
			...
			$course_details_html    = '<h2>' . esc_html__( 'Course details', 'sensei-wc-paid-courses' ) . '</h2>';
			...
			if ( $courses && count( $courses ) > 0 ) {
				...
				foreach ( $courses as $course ) {
					...
					$course_details_html .= '<p><strong>' . sprintf( __( 'View course: %1$s', 'sensei-wc-paid-courses' ), '</strong><a href="' . esc_url( $permalink ) . '">' . esc_html( $title ) . '</a>' ) . '</p>';
				}
			}
